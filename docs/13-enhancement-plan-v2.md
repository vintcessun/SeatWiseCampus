...
- **相关文件**：[docs/14-known-issues-v3.md](14-known-issues-v3.md)。
...
## UI 升级 · 微交互与视觉（✅ 已实现 2026-07-09）
对标 2026 UI 趋势（克制玻璃拟态 + 有目的的动效），在**不改变信息架构**的前提下叠加视觉层：
- **登录页**：漂浮极光光斑背景 + 玻璃拟态登录卡（`Login.vue`）。
- **数字滚动**：可复用 `components/CountUp.vue`（easeOutCubic），用于学生/管理端概览与自习报告 KPI。
- **成功彩带**：无依赖 `utils/confetti.js`（canvas 粒子），预约/组队/候补确认/签到成功时喷射。
- **全局微交互**：卡片悬浮抬升 + 柔和阴影、座位悬停放大、路由淡入滑动过渡、背景径向渐变（`styles.css` + `App.vue` + 布局过渡）。
- 均为纯前端、无新增依赖，不超出原设计范围。

## 深色模式主题系统（✅ 已实现 2026-07-09）
- 头部一键切换明亮/深色，`localStorage` 持久化；`utils/theme.js` 在 `<html>` 上加/去 `.dark`。
- 接入 Element Plus 官方暗色 CSS 变量（`theme-chalk/dark/css-vars.css`）自动适配 el-* 组件；`styles.css` 补齐定制表面（页头、座位配色、概览/报告数字、登录面板）的深色覆盖，并加平滑过渡。
- 整站沉浸式深色，专业高级感；纯前端、无新增依赖。

## 时空座位图（空间+时间联合查询）（✅ 已实现 2026-07-09）
> **已知缺陷**：管理端缺少时空座位图；学生端时空座位图与选座页脱节，不能在时间轴上直接预约（详见 [docs/14-known-issues-v3.md](14-known-issues-v3.md) §R7）。

落地 [docs/10](10-校园座位预约系统-改进建议.md) §一 的旗舰构想，**复用历史回放接口、无需新增后端**：
- 学生「时空座位图」页（`views/student/Spacetime.vue`）：房间/日期选择 + **时间轴滑块**（可播放一天）。
- 拖动时间轴选择**预约开始时刻**，每个座位按「从该时刻起的连续可用时长」做**绿色渐变发光**（浅绿<30m → 深绿≥3h 带辉光），占用座位置灰红。回答"哪个座位现在空闲、又能连续坐 2 小时"。
- 点击发光座位 → 弹出该座当日时段条 + 「以此刻为开始去预约」→ 跳转选座页并 **query 预填** 日期/开始时间（`Seats.vue` 读取 `route.query`）。
- 右侧全天占用曲线；纯前端聚合 `replay` 时间片矩阵。无新增依赖/后端。
