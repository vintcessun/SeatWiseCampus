...
async function shot(port, who, path, file, wait = 2600) {
  const userDir = join(os.tmpdir(), 'sw-ann-' + port + '-' + Date.now())
  const chrome = spawn(CHROME, ['--headless=new', '--disable-gpu', '--no-sandbox', '--no-first-run', '--hide-scrollbars', '--remote-debugging-port=' + port, '--user-data-dir=' + userDir, '--window-size=1440,1040', '--lang=zh-CN', 'about:blank'], { stdio: 'ignore' })
  await sleep(2500)
  const cdp = await CDP.attach(port)
  await cdp.send('Page.enable'); await cdp.send('Runtime.enable')
  const ev = e => cdp.send('Runtime.evaluate', { expression: e, returnByValue: true })
  const goto = async (u, w = 2600) => { await cdp.send('Emulation.setDeviceMetricsOverride', { width: 1440, height: 1040, deviceScaleFactor: 2, mobile: false }); await cdp.send('Page.navigate', { url: u }); await sleep(w) }
...
async function main() {
  ...
  await shot(9235, stu, '/student/home', '23-announce-student.png')
  await shot(9236, adm, '/admin/announcements', '24-announce-admin.png')
  process.exit(0)
}
