...
async function main() {
  ...
  const T = tomorrow()
  for (const [st, en] of [['14:00', '16:00'], ['18:00', '19:00']]) {
    const bd = (await api(`/api/study-rooms/1/board?date=${T}&start=${st}&end=${en}`, { token: stu.token })).data
    const seat = bd.seats.find(s => s.status === 'FREE')
    if (seat) await api('/api/reservations', { method: 'POST', token: stu.token, body: { roomId: 1, seatId: seat.seatId, date: T, startTime: st, endTime: en } })
  }
  const userDir = join(os.tmpdir(), 'sw-home-' + Date.now())
  const chrome = spawn(CHROME, ['--headless=new', '--disable-gpu', '--no-sandbox', '--no-first-run', '--hide-scrollbars', '--remote-debugging-port=9227', '--user-data-dir=' + userDir, '--window-size=1440,1000', '--lang=zh-CN', 'about:blank'], { stdio: 'ignore' })
  await sleep(2500)
  const cdp = await CDP.attach(9227)
  ...
  await goto(WEB + '/student/home', 2600)
  const m = await cdp.send('Page.getLayoutMetrics'); const cs = m.cssContentSize || m.contentSize
  await cdp.send('Emulation.setDeviceMetricsOverride', { width: Math.ceil(cs.width), height: Math.min(Math.ceil(cs.height), 1100), deviceScaleFactor: 2, mobile: false }); await sleep(300)
  const res = await cdp.send('Page.captureScreenshot', { format: 'png', captureBeyondViewport: true, fromSurface: true })
  writeFileSync(join(__dirname, 'shots', '17-student-home.png'), Buffer.from(res.data, 'base64'))
  console.log('📸 17-student-home.png')
