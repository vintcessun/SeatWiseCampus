...
${shot('20-group.png', '⑧ 组队相邻预约（原子多座）', '一次为多名成员预约同一时段相邻座位；有序 Redisson 锁+单事务+唯一索引，全部成功或整体回滚，演示分布式并发原子性')}
${shot('21-replay.png', '⑨ 座位历史回放', '拖动播放条重建当天座位占用轨迹，利用率仪表盘+全天占用曲线，一眼定位最拥挤时刻；由预约记录按时间片重建，无需事件表')}
${shot('22-mine-locate.png', '⑩ 选座页「我的预约」定位', '选座页直接展示本自习室今日待签到/使用中的预约，点击胶囊在座位图中高亮定位，无需跳转「我的预约」')}
${shot('23-announce-student.png', '⑪ 公告中心 · 学生端横幅', '管理员发布的系统公告在学生首页顶部横幅展示（通知/提醒两级），并可在发布时一并推送站内通知')}
${shot('24-announce-admin.png', '⑫ 公告中心 · 管理端', '管理端公告 CRUD：发布/编辑/上下线/删除，发布时可勾选「同时推送给所有学生」')}
${shot('25-study-report.png', '⑬ 个人自习报告', '基于预约/签到记录聚合：累计场次与时长、连续自习天数、守约率、近 7 天时长柱状图与守约环图（ECharts）')}
${shot('26-login-aurora.png', '⑭ UI 升级 · 极光玻璃登录页', '2026 视觉趋势落地：漂浮极光光斑背景 + 玻璃拟态卡片；全局叠加数字滚动计数、预约/签到成功彩带、卡片悬浮与路由过渡等微交互')}
${shot('27-dark-student.png', '⑮ 深色模式 · 学生端', '一键切换明亮/深色主题（持久化）：接入 Element Plus 暗色变量 + 定制表面/座位配色，整站沉浸式深色')}
${shot('28-dark-board.png', '⑯ 深色模式 · 管理端实时看板', '深色下座位热力与实时事件流依旧清晰，KPI 高亮，专业感与高级感兼具')}
${shot('29-spacetime.png', '⑰ 时空座位图（空间+时间联合查询）', '拖动时间轴选预约开始时刻，座位按「从该时刻起的连续可用时长」绿色渐变发光，占用置灰红；一眼找到既空闲又能久坐的位置，点击带时刻跳转选座页一键预约')}
${shot('30-spacetime-dark.png', '⑱ 时空座位图 · 深色', '深色主题下的时空热力，发光座位更具沉浸感；复用历史回放接口，无需新增后端')}
${shot('32-noti-types.png', '⑲ 已知问题 V3 修复（R9/R4/R10 等）', '通知类型正确区分并配图标（修复 R9）；自习室临时关闭联动公告通知受影响学生（R4/R10）；停用座位/重排校验未来预约')}
${shot('33-pomodoro.png', '⑳ 专注番茄钟（R12）', '学生端番茄工作法：25/5/15 循环、环形倒计时、完成彩带+提示音、当日番茄与专注时长统计')}
${shot('35-layout-edit.png', '㉑ 排布编辑器·手动模式（R5）', '画笔逐格绘制 座位/过道/空位/禁用位，支持多过道与不规则房间；+行/+列；保存自动按行编号')}
${shot('36-admin-spacetime.png', '㉒ 管理端时空占用图（R7）', '管理端复用时空视图（只读）：拖动时间轴查看任意时刻的空间占用分布')}
${shot('37-register-captcha.png', '㉓ 注册图形验证码（R1）', 'SVG 图形验证码：随机 4 字符、彩色旋转 + 干扰线；Redis 存 5 分钟、一次性、大小写不敏感，防自动化注册')}
${shot('38-locations.png', '㉔ 位置管理（R11）', '管理端维护楼栋经纬度；配合学生端「定位我的位置」（浏览器 Geolocation + Haversine）就近推荐自习室')}
${shot('39-admins.png', '㉕ 管理员管理 · 子管理员（R6）', '主管理员(SUPER)可创建/删除子管理员(ADMIN_SUB)；子管理员可管理自习室/座位/公告/报表，但不能管理其他管理员')}
