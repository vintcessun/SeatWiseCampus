...
async function main() {
  ...
  const userDir = join(os.tmpdir(), 'sw-dark-' + Date.now())
  const chrome = spawn(CHROME, ['--headless=new', '--disable-gpu', '--no-sandbox', '--no-first-run', '--hide-scrollbars', '--remote-debugging-port=9242', '--user-data-dir=' + userDir, '--window-size=1440,900', '--lang=zh-CN', 'about:blank'], { stdio: 'ignore' })
  await sleep(2500)
  const cdp = await CDP.attach(9242)
  await cdp.send('Page.enable'); await cdp.send('Runtime.enable')
  const ev = e => cdp.send('Runtime.evaluate', { expression: e, returnByValue: true })
  const goto = async (u, w = 2600) => { await cdp.send('Emulation.setDeviceMetricsOverride', { width: 1440, height: 900, deviceScaleFactor: 2, mobile: false }); await cdp.send('Page.navigate', { url: u }); await sleep(w) }
  await goto(WEB + '/login', 1000)
  // 开启深色主题 + 登录态（在导航前写入 localStorage）
  await ev(`localStorage.setItem('sw-theme','dark');localStorage.setItem('satoken','${stu.token}');localStorage.setItem('role','${stu.role}');localStorage.setItem('userInfo',JSON.stringify(${JSON.stringify(stu.userInfo)}))`)
  // 学生首页（深色）
  await goto(WEB + '/student/home', 3800)
  await sleep(800)
  let res = await cdp.send('Page.captureScreenshot', { format: 'png', fromSurface: true })
  writeFileSync(join(__dirname, 'shots', '27-dark-student.png'), Buffer.from(res.data, 'base64'))
  console.log('📸 27-dark-student.png')
  // 管理端看板（深色）
  ...
  await goto(WEB + '/admin/rooms/1/board', 3800)
  await sleep(800)
  res = await cdp.send('Page.captureScreenshot', { format: 'png', fromSurface: true })
  writeFileSync(join(__dirname, 'shots', '28-dark-board.png'), Buffer.from(res.data, 'base64'))
  console.log('📸 28-dark-board.png')
