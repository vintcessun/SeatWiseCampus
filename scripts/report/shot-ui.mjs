...
async function main() {
  ...
  const userDir = join(os.tmpdir(), 'sw-ui-' + Date.now())
  const chrome = spawn(CHROME, ['--headless=new', '--disable-gpu', '--no-sandbox', '--no-first-run', '--hide-scrollbars', '--remote-debugging-port=9238', '--user-data-dir=' + userDir, '--window-size=1440,900', '--lang=zh-CN', 'about:blank'], { stdio: 'ignore' })
  await sleep(2500)
  const cdp = await CDP.attach(9238)
  ...
  // 登录页（极光玻璃）
  await goto(WEB + '/login', 2600)
  let res = await cdp.send('Page.captureScreenshot', { format: 'png', fromSurface: true })
  writeFileSync(join(__dirname, 'shots', '26-login-aurora.png'), Buffer.from(res.data, 'base64'))
  console.log('📸 26-login-aurora.png')
  process.exit(0)
}
